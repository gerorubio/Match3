...
function PlayState:update(dt)
    ...
    if self.canInput then
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
                ...
                :finish(function()
                    m = self:calculateMatches()
                    if m == 0 then
                        local tile1 = self.board.tiles[y][x]
                        local tile2 = self.board.tiles[tempY][tempX]
                        local auxX = tile1.gridX
                        local auxY = tile1.gridY
                        ...
                        self.board.tiles[y][x] = tile2
                        self.board.tiles[tempY][tempX] = tile1
                        gSounds['error']:play()
                        ...
                        continue = self.board:possibleMove()
    ...
    --Update for partible system to work for shiny tile
    ...
    if matches then
        ...
        for k, match in pairs(matches) do
            for i = 1, #match do
                ...
                if match[i].shiny then
                    match[i].shiny = false
                    auxMatch = {}
                    if math.random() > 0.5 then
                        yShiny = match[i].gridY
                        for j = 1, 8 do
                            table.insert(auxMatch ,self.board.tiles[yShiny][j])
                        end
                    else
                        xShiny = match[i].gridX
                        for j = 1, 8 do
                            table.insert(auxMatch ,self.board.tiles[j][xShiny])
                        end
                    end
                    table.insert(matches, auxMatch)
                end
            end
            plusTime = plusTime + #match
        end
        
        self.timer = self.timer + plusTime

        -- remove any tiles that matched from the board, making empty spaces
        -- for every tile remove we add a second to the timer
